    ...
    async def handle(self, request):  # type: ignore[override]
        ...
        try:
            if upstream_resp.headers.get("content-type", "").startswith("application/json"):
                ...
                logger.info(f"[{correlation_id}] ⬅︎ Body: {redact_sensitive_data(body) if isinstance(body, dict) else body}")
        except Exception:
            pass
...
class TransparentOAuthProxyProvider(OAuthAuthorizationServerProvider[AuthorizationCode, Any, AccessToken]):
    ...
    async def register_client(self, client_info: OAuthClientInformationFull) -> None:  # noqa: D401
        ...
        client_info.token_endpoint_auth_method = (
            "none" if self._s.client_secret is None else "client_secret_post"
        )
    ...
    async def load_authorization_code(self, client: OAuthClientInformationFull, authorization_code: str) -> AuthorizationCode | None:  # noqa: D401,E501
    ...
    async def exchange_authorization_code(self, client: OAuthClientInformationFull, authorization_code: AuthorizationCode) -> OAuthToken:  # noqa: D401,E501
    ...
    async def load_access_token(self, token: str) -> AccessToken | None:  # noqa: D401
        ...
        return AccessToken(token=token, client_id=str(self._s.client_id), scopes=[self._s.default_scope], expires_at=None)
