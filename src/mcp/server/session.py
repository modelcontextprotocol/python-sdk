...
This module provides the ServerSession class, which manages communication
between the server and client in the MCP (Model Context Protocol) framework.
It is most commonly used in MCP servers to interact with the client.
...
The ServerSession class is typically used internally by the Server class
...
# Only import the checkpoint backend for type checking to avoid runtime
# dependencies or circular imports. The actual implementation lives in
...
class ServerSession(
    ...
    def checkpoint_backend(self) -> "CheckpointBackend | None":
        ...
        FastMCP or custom servers can use this to implement the
        checkpoint/create, checkpoint/validate, checkpoint/resume,
        checkpoint/delete methods by delegating into the backend and using
        this session as the context.
        """
