...
Key SEP-2663 rules implemented here:
  ...
  - `tasks/get` returns the task (`resultType: "complete"`), inlining the original
    `CallToolResult`. A tool result with `isError: true` is a `completed` task,
    not `failed` (`failed` is reserved for JSON-RPC errors).
...
recorded directly in `completed` status -- SEP-2663 allows any initial status (the
embedded task is "typically (though not necessarily)" `working`). A task exists
only once its outcome exists: an error response never strands a half-created
task, there is no `working` state to corrupt, and no terminal transition to
guard. Background execution (returning `working` tasks), the in-task
...
from mcp_types import INVALID_PARAMS, EmptyResult, ErrorData, RequestParams, Result
...
"""SEP-2663 task statuses. Inline execution only ever records `completed`."""
...
class TaskRecord:
    ...
    `task` is the wire snapshot, `result` the serialized `CallToolResult` it
    completed with, and `expires_at` the absolute deadline derived from `ttlMs`
    (`None` = never expires).
    """
...
class Tasks(Extension):
    ...
    Args:
        ...
        default_ttl_ms: Retention for completed tasks, in milliseconds, stamped
    ...
    async def intercept_tool_call(
        ...
        # A raise propagates as the JSON-RPC error the client is already waiting
        # on: no task was announced, so none is recorded (`failed` is reserved for
        # background execution, where an error has a task to land on).
        result = await call_next(ctx)
        if isinstance(result, ErrorData):
            # A nested extension returned the error instead of raising; surface it
            # the way the runner would rather than minting a "completed" task.
        ...
        if payload.get("resultType") == "input_required":
            ...
            return result
        # A tool result -- even `isError: true` -- is a completed task.
        now = self._clock()
        ...
        # Store a copy: the chain's dict must not alias the durable record.
        await self._store.put(TaskRecord(task=task, result=deepcopy(payload), expires_at=expires_at))
    ...
    async def _handle_get(self, ctx: ServerRequestContext[Any, Any], params: GetTaskRequestParams) -> HandlerResult:
        ...
        # Every stored task is `completed`, so the original CallToolResult is
        # always inlined; serve a copy so a caller mutating the response cannot
