"""Starlette middleware that enforces `TransportSecuritySettings` on HTTP transports.

Kept apart from `mcp.server.transport_security` (which defines only the
web-framework-free settings model, so `import mcp.server` and stdio servers
never load starlette). `mcp.server.transport_security` still re-exports
`TransportSecurityMiddleware` for existing imports of it.
"""
