"""The one interpreter for the per-version fact blocks.
...
Serialization order is fixed: refuse, dump, strip, inject, required `_meta`.
Refusals fire before any reshaping so a refused value never half-shapes.
Injections and the `_meta` step apply to the top-level model only, and the
strip walk never descends into an embedded message body (request,
notification, or result models nested inside another body's fields, such as
input-request/input-response map values): embedded payloads pass through
verbatim, and their hygiene is the embedding caller's responsibility.
...
entirely when the version is unknown (`facts` is None).
"""
...
_MESSAGE_BODY_TYPES = (Request, Notification, Result)
...
def _is_dict(value: Any) -> TypeGuard[dict[str, Any]]:
    """isinstance check that narrows a dynamic JSON value to a typed dict."""
...
def serialize(model: BaseModel, version: str, facts: VersionFacts) -> dict[str, Any]:
    ...
    dump = _dump(model)
    _strip(model, dump, facts.strip_on_emit)
...
def _wire_items(model: BaseModel) -> Iterator[tuple[str, Any]]:
    """(wire key, field value) for every set field of `model`."""
    for name, field in type(model).model_fields.items():
        value = getattr(model, name)
        if value is not None:
            yield field.serialization_alias or field.alias or name, value


def _strip(model: BaseModel, dump: dict[str, Any], rows: tuple[Strip, ...]) -> None:
    """Drop every row-named wire field from `dump`, walking nested models.

    The model and its dump are walked in parallel so rows match by isinstance
    at every level with no path bookkeeping. The walk skips embedded message
    bodies (nested request/notification/result models): their user-set fields
    pass through verbatim.
    """
    for row in rows:
        if isinstance(model, row.owner):
            dump.pop(row.wire_field, None)
    for wire_key, value in _wire_items(model):
        _strip_value(value, dump.get(wire_key), rows)


def _strip_value(value: Any, dump_value: Any, rows: tuple[Strip, ...]) -> None:
    """Recurse `_strip` through one field value (model, list, or mapping)."""
    if isinstance(value, BaseModel):
        if not isinstance(value, _MESSAGE_BODY_TYPES) and _is_dict(dump_value):
            _strip(value, dump_value, rows)
    elif _is_list(value) and _is_list(dump_value):
        for item, dump_item in zip(value, dump_value, strict=False):
            _strip_value(item, dump_item, rows)
    elif _is_dict(value) and _is_dict(dump_value):
        for key, item in value.items():
            _strip_value(item, dump_value.get(key), rows)
...
def _inject(model: BaseModel, dump: dict[str, Any], rows: tuple[Inject, ...]) -> None:
    ...
    own value for a field its base would otherwise default.
    """
    mro = type(model).__mro__
    chosen: dict[str, Inject] = {}
    for row in rows:
        if not isinstance(model, row.owner):
            continue
...
def parse(type_: Any, data: Mapping[str, Any], version: str, facts: VersionFacts | None) -> Any:
...
def _apply_parse_mandates(value: Any, data: Mapping[str, Any], version: str, facts: VersionFacts) -> None:
...
def _select_result_members(
    arms: tuple[type[Result], ...], data: Mapping[str, Any], facts: VersionFacts | None
) -> tuple[type[Result], ...]:
