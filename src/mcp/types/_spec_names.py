...
always map one-to-one onto the SDK's public names. This module is the single
record of every deliberate divergence, consumed by the spec-comparison tests —
an entry here is a reviewed decision, not drift.
...
- ``SDK_ONLY_NAMES``: public ``mcp.types`` names with no schema counterpart in
...
SCHEMA_NOT_MODELED: Final[dict[str, str]] = {
    ...
    # `_meta` containers: the SDK carries them as open mappings (`Meta`,
    # `RequestParamsMeta`), and the wire boundary manages the reserved keys.
    ...
    # Alternative considered: modeling the requested-schema vocabulary as typed classes alongside the untyped field.
