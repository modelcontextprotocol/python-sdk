...
Every section heading names the API it affects, so searching this page for the symbol your code uses is the fastest route to the change that broke it. Only breaking and behavior changes are listed.
...
| Tool exception messages no longer reach the model | tool error text is only `Error executing tool <name>`, without the exception message | [exception messages](#exception-messages-from-mcptool-handlers-no-longer-reach-the-model) |
...
| If you... | Read |
|---|---|
| pin or cap dependencies, or use `httpx` directly | [Packaging and dependencies](#packaging-and-dependencies) |
...
| rely on lenient handling of off-schema traffic, or assert on exact wire bytes | [Stricter protocol validation and wire behavior](#stricter-protocol-validation-and-wire-behavior) |
...
1. Update your dependency pins: [Packaging and dependencies](#packaging-and-dependencies).
...
Direct `httpx-sse` usage becomes `httpx2.EventSource` or `AsyncClient.sse()`. Code that imported `httpx` or `httpx_sse` only because v1 pulled them in now fails with `ModuleNotFoundError: No module named 'httpx'` (or `'httpx_sse'`): declare `httpx` in your own dependencies (the two packages install side by side) or port those calls.
...
The objects are not interchangeable either: an `httpx.AsyncClient` passed as `http_client=` still connects and calls tools, but its standalone GET stream never opens, so messages the server sends outside an in-flight request (a list-changed notification, say) silently never arrive.
...
`mcp.shared.version` is gone: its constants now live in `mcp.types.version` and changed meaning as well; see [`LATEST_PROTOCOL_VERSION` and `SUPPORTED_PROTOCOL_VERSIONS` changed](#latest_protocol_version-and-supported_protocol_versions-changed).
...
The nested `RequestParams.Meta` Pydantic model is now the `RequestParamsMeta` TypedDict (`from mcp.types import RequestParamsMeta`), and `NotificationParams.Meta` is gone: notification `_meta` is a plain `dict[str, Any]`. `ctx.meta` (lowlevel handlers, client callbacks) and `ctx.request_context.meta` (`MCPServer` handlers) are dicts, so attribute access becomes key access, `progressToken` becomes the optional `progress_token` key, and `_meta` you built with `types.RequestParams.Meta(...)` or `types.NotificationParams.Meta(...)` is passed as a plain dict (`CallToolRequestParams(name="x", _meta={"traceparent": ...})`). The wire format is unchanged.

If you read `progressToken` only to send progress, call `report_progress()` instead: it is a no-op when the caller did not ask for progress, and it also delivers on the default in-process `Client(server)` connection, where `ctx.meta` carries no `progress_token` even when the caller passed `progress_callback` (see [Progress](handlers/progress.md)).
...
```python
await ctx.session.report_progress(0.5, 100)  # ctx: ServerRequestContext
```
...
`mcp.shared.version` is gone (`ModuleNotFoundError`); its constants live in `mcp.types.version` and changed under the same names:

- `LATEST_PROTOCOL_VERSION` was `"2025-11-25"`, the version the v1 client offered in `initialize`. In v2 it is `"2026-07-28"`, the newest revision the SDK speaks, which the `initialize` handshake never negotiates (see [Protocol versions](protocol-versions.md)); `from mcp.types import LATEST_PROTOCOL_VERSION` still imports, so the value changes silently. Where your code used it as the handshake version (a hand-built `initialize`, or a comparison against the negotiated version), use `LATEST_HANDSHAKE_VERSION` (`"2025-11-25"`).
- `SUPPORTED_PROTOCOL_VERSIONS` is now a `tuple` (was a `list`) with `"2026-07-28"` appended: membership tests still pass, but `[-1]` is `"2026-07-28"` and list operations such as `+ [...]` raise `TypeError`. The handshake-only set, with v1's four values, is `HANDSHAKE_PROTOCOL_VERSIONS`.
...
`FastMCP` is now `MCPServer`, and the `mcp.server.fastmcp` package is now `mcp.server.mcpserver`; submodule paths below it are unchanged. The old path is removed, not aliased: importing `mcp.server.fastmcp` or anything below it raises `ModuleNotFoundError`, so an `except ImportError` fallback in code that supports both majors keeps working.
...
`MCPServer` is also exported from `mcp.server`, as `FastMCP` was. Other imports move the same way:
...
- `Message`, `UserMessage`, `AssistantMessage` from `mcp.server.mcpserver.prompts.base` (now also re-exported from `mcp.server.mcpserver`)
...
The `ctx.fastmcp` property is now `ctx.mcp_server`. Beyond the changes in this group, the `@mcp.tool()`/`@mcp.resource()`/`@mcp.prompt()` decorators, the `lifespan=` hook, and `mcp.run()` port with only the import change.

### Default server name and version changed

A nameless `FastMCP()` reported `serverInfo.name == "FastMCP"`; a nameless `MCPServer()` reports `"mcp-server"`. Without `version=` (v1's `FastMCP` had no such parameter), `serverInfo.version` was the installed `mcp` package version; v2 reports an empty string. Nothing raises, but clients keying off `serverInfo` and tests asserting on the initialize result see the new values, so set both explicitly:
...
`mount_path` is gone from the constructor, `Settings`, `run()`, `run_sse_async()`, and `sse_app()`: passing it raises `TypeError`, and `mcp.settings.mount_path = ...` raises `ValueError`. The SSE transport prefixes its advertised message endpoint with the ASGI `root_path` that Starlette's `Mount` sets, so delete `mount_path` and mount as before.
...
`mcp.run(transport="sse", mount_path="/x")` becomes `mcp.run(transport="sse")`; if a reverse proxy strips `/x`, serve with `uvicorn.run(mcp.sse_app(), root_path="/x")` instead so the advertised message endpoint keeps the prefix. See [Add to an existing app](run/asgi.md) for mounting details.
...
Moved: `host`, `port`, `sse_path`, `message_path`, `streamable_http_path`, `json_response`, `stateless_http`, `max_request_body_size`, `session_idle_timeout`, `max_sessions`, `event_store`, `retry_interval`, `transport_security`. `mount_path` is removed, not moved. Each transport takes only its own keywords (`run()` is overloaded per transport, so a type checker rejects e.g. `run(transport="sse", json_response=True)` and at runtime it raises `TypeError`); [Running your server](run/index.md) lists them. `json_response=True` and `stateless_http=True` also change how server-to-client requests fail, see [Server-initiated sampling, elicitation, and roots raise `NoBackChannelError`](#server-initiated-sampling-elicitation-and-roots-raise-nobackchannelerror).
...
The moved fields are gone from `mcp.settings` too: v1's `mcp.settings.port = 9000` or `mcp.settings.streamable_http_path = "/"` before `run()` / mounting now raises `ValueError: "Settings" object has no field "port"` (or `"streamable_http_path"`); write `run(..., port=9000)` or `streamable_http_app(streamable_http_path="/")` instead. If another module builds the app, pass the keywords there (e.g. `functools.partial(mcp.streamable_http_app, json_response=True)`).
...
Under streamable HTTP, v1 entered the server's `lifespan` for every session (stateful) or every request (`stateless_http=True`) and exited it when that session or request ended. In v2 `StreamableHTTPSessionManager.run()` enters it once at startup and exits it at shutdown; the object it yields is the same `ctx.request_context.lifespan_context` for every session and request (see [Lifespan](handlers/lifespan.md)). SSE and stdio are unchanged.

State you scoped to a session or request by yielding it from the lifespan (a per-client cache, a per-connection transaction) is now shared between all clients and torn down only at server shutdown. There is no per-session teardown hook: keep only server-wide resources (connection pool, HTTP client, loaded model) in the lifespan, acquire and release per-request state inside the handler, and key anything that must persist across one client's stateful session on the `mcp-session-id` header (`ctx.headers`; see [`ServerSession` is now a thin proxy](#serversession-is-now-a-thin-proxy-no-longer-a-basesession)).

### `MCPServer.get_context()` removed

`MCPServer.get_context()` is gone; there is no ambient context to fetch. Declare a `ctx: Context` parameter on the handler and the SDK injects it. Nothing is injected into functions the handler calls, so helpers that used `get_context()` now take `ctx` as an argument. See [The Context](handlers/context.md).
...
v1 also awaited a coroutine returned by a resource or prompt function; v2 only checks whether the function itself is async, so an `async def` resource or prompt wrapped by a synchronous decorator is no longer awaited and the client receives its `<coroutine object ...>` repr as the content. Make the wrapper `async def` as well.

### Calling `MCPServer.call_tool()`, `get_prompt()`, and `read_resource()` directly

Direct calls to these methods (unit tests, wrapper servers, a handler that calls another) change in three ways.
...
`get_prompt()` is likewise typed `GetPromptResult | InputRequiredResult` and `read_resource()` is `Iterable[ReadResourceContents] | InputRequiredResult`; handlers ported from v1 never produce `InputRequiredResult`, so only the narrowing is new (`assert not isinstance(result, InputRequiredResult)` suffices; `Prompt.render()` and `ResourceTemplate.create_resource()` widen too). In tests, prefer the in-memory [`Client(mcp)`](get-started/testing.md), whose methods return plain `CallToolResult`/`GetPromptResult`/`ReadResourceResult`.

**Exceptions.** A tool that raises anything other than `ToolError`, `ResourceError`, or `MCPError` now surfaces from `call_tool()` as `UnexpectedToolError`, a `ToolError` subclass whose message is only `Error executing tool <name>` (v1 appended `: <original message>`); `except ToolError` still catches it, and the original exception is on `__cause__`. An `MCPError` raised by the tool now propagates unchanged instead of being wrapped in `ToolError` (see [Handling errors](servers/handling-errors.md)). Direct `read_resource()` failures raise `ResourceError` subclasses instead of `ValueError` (see [Reading a missing resource raises `ResourceNotFoundError` and returns invalid-params](#reading-a-missing-resource-raises-resourcenotfounderror-and-returns-invalid-params)).

**No ambient request context.** v1 read the ambient request context, so `mcp.call_tool()` from inside a handler gave the inner handler a working `ctx`. In v2 each method takes an optional `context: Context | None = None` and builds a request-less [`Context`](handlers/context.md) when you omit it, so in the inner handler `ctx.session`, `ctx.request_id`, `ctx.report_progress()`, etc. raise `ValueError("Context is not available outside of a request")`. Pass the caller's `ctx` through:
...
The lower layers (`ToolManager.call_tool`, `Tool.run`, `PromptManager.render_prompt`, `Prompt.render`, `ResourceTemplate.create_resource`, `ResourceManager.get_resource`) now require the `context` argument; call the `MCPServer` methods above instead.

### `MCPError` raised from an `@mcp.tool()` handler now surfaces as a JSON-RPC error

Raising `MCPError` (or a subclass) inside an `@mcp.tool()` handler, or letting one propagate from a call the tool makes, now fails the `tools/call` request with a JSON-RPC error carrying the raised `code`, `message`, and `data`, so the client's `call_tool()` raises `MCPError` instead of returning a result. v1 caught it like any other exception and returned `CallToolResult(isError=True)` with the text `Error executing tool <name>: <message>`, dropping the code and `data` (`UrlElicitationRequiredError` was already re-raised and is unchanged).
...
Raise `MCPError` when the request itself should be rejected. To hand the model an `is_error=True` result instead, raise `ToolError` or return `CallToolResult(is_error=True, ...)`; any other exception still produces one, but with only the generic text `Error executing tool <name>` (next section).

### Exception messages from `@mcp.tool()` handlers no longer reach the model

v1 turned any exception raised inside an `@mcp.tool()` function into `CallToolResult(isError=True)` with the text `Error executing tool <name>: <exception message>`, so the model (and tests asserting on `content[0].text`) saw the message. v2 keeps the message only for `ToolError`; `MCPError` [becomes a JSON-RPC error](#mcperror-raised-from-an-mcptool-handler-now-surfaces-as-a-json-rpc-error), and any other exception is treated as a crash: the result is still `is_error=True` but its text is just `Error executing tool <name>`, and the traceback goes to the server log at `ERROR`. Raise `ToolError` for failures the model should read (v1 has the same class at `mcp.server.fastmcp.exceptions.ToolError` with identical output, so the change can land before upgrading), or return `CallToolResult(is_error=True, ...)`.
...
# isError=True, text:
# "Error executing tool read_config: [Errno 2] No such file or directory: 'app.toml'"
```
...
def read_config(path: str) -> str:
    try:
        ...
        raise ToolError(f"Cannot read {path}: {e.strerror}") from e

# is_error=True, text:
# "Error executing tool read_config: Cannot read app.toml: No such file or directory"
```

A direct `await mcp.call_tool(...)` on a crashing tool raises `UnexpectedToolError` with the same generic message (see [Calling `MCPServer.call_tool()`, `get_prompt()`, and `read_resource()` directly](#calling-mcpservercall_tool-get_prompt-and-read_resource-directly)). Prompt, resource, and `@mcp.completion()` functions follow the same rule: only `MCPError` (or `ResourceError` from a resource) carries your message to the client; any other exception yields a generic error message and a server-side traceback. See [Handling errors](servers/handling-errors.md).

### Content-block return annotations no longer produce `output_schema` or `structured_content`

A v1 tool whose return annotation is or contains a content-block type (`-> TextContent`, `-> list[TextContent]`, `-> EmbeddedResource`, `-> list[str | ImageContent]`, ...) advertised an `outputSchema` and echoed its return value into `structuredContent`. `MCPServer` treats these annotations as unstructured: the tool lists with `output_schema=None` and returns `structured_content=None`. `content` is unchanged.

**Before (v1):**

```python
@mcp.tool()
def report() -> list[TextContent]:
    return [TextContent(type="text", text="done")]

result = await session.call_tool("report", {})
result.structuredContent  # {"result": [{"type": "text", "text": "done", ...}]}
```
...
result.structured_content  # None; result.content is unchanged
```

Read `content`, or pass `@mcp.tool(structured_output=True)` to keep publishing the schema and `structured_content`. See [Structured output](servers/structured-output.md#content-blocks-and-media).

### Reading a missing resource raises `ResourceNotFoundError` and returns invalid-params

Reading a URI that matches no resource or template now fails with JSON-RPC code `-32602` (invalid params) and `error.data = {"uri": ...}`, per [SEP-2164](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2164); v1 returned code `0` with no `data`. Other read failures return `-32603` with the same `data`: a `ResourceError` raised by the handler keeps its message, while any other exception gets a generic message naming only the URI (v1 sent the exception text; v2 logs it server-side).

Server-side, `MCPServer.read_resource()` and `ctx.read_resource()` raise `ResourceNotFoundError` for an unknown URI and its parent `ResourceError` for every other failure (a crashing handler is wrapped in the subclass `UnexpectedResourceError`, original on `__cause__`). None of these subclasses `ValueError`, which v1 raised for an unknown URI or a failing template, so an unported `except ValueError` inside a tool now lets the error escape and the tool returns `is_error=True`.
...
A resource or template handler that raised a plain exception for a missing instance should now raise `ResourceNotFoundError` so its message and the URI reach the client as `-32602`; see [Handling errors](servers/handling-errors.md).

### `Resource` classes reject unknown keyword arguments

The `Resource` base class now sets `extra="forbid"`, so `TextResource`, `BinaryResource`, `FunctionResource`, `FileResource`, `HttpResource`, `DirectoryResource`, and your own subclasses raise `pydantic.ValidationError` on an unrecognized keyword argument instead of silently dropping it. Remove the stray argument; to restore v1's silent-drop behavior on a subclass, set `model_config = ConfigDict(extra="ignore")`.
...
Two defaults change silently: files whose mime type is textual but not `text/*` (`application/json`, `application/xml`, `image/svg+xml`, …), which v1 served as blobs, are now served as text; and text files are decoded as UTF-8 rather than with the platform locale encoding (v1 called `Path.read_text()` with no encoding), so a non-UTF-8 file that v1 only read correctly under a matching locale (e.g. cp1252 on Windows) now raises `UnicodeDecodeError`. Where either default is wrong for a file, pass `encoding` explicitly.
...
**Literals and delimiters match exactly.** `data://v1.0/{id}` no longer matches `data://v1X0/42` (a template `.` was regex any-character), and `{var}` no longer captures `?`, `#`, `&`, or `,`: `api://{id}` no longer matches `api://foo?x=1`. To keep accepting the query string, write `api://{id}{?x}` and give the `x` parameter a default; for values that contain a literal `,` or `&`, use `{+var}` (which also admits `/`) or have clients percent-encode them.
...
On the `MCPServer` `Context`, `log()`, `debug()`, `info()`, `warning()`, and `error()` now take `data: Any` (any JSON-serializable value) instead of `message: str`; calls that pass `message=` by keyword raise `TypeError`.
...
The `mcp.shared.progress` module (`Progress`, `ProgressContext`, and the `progress()` context manager) is removed. Use `Context.report_progress()` in an `MCPServer` handler instead. Two behavior changes:
...
`ctx.elicit()` (and `elicit_with_validation()`) now reject two field shapes v1 accepted, raising `TypeError` before anything is sent: a `list[str]` field without spec-valid enum `items` and a union of primitives such as `int | str`. Use `list[Literal[...]]` for a multi-select and a single primitive type per field.
...
`ElicitationResult` (the return type of `ctx.elicit()`) is now a subscriptable `TypeAliasType` rather than a plain union, so it is no longer a valid `isinstance()` target. Check the member classes instead, or branch on `result.action` (`"accept"` / `"decline"` / `"cancel"`), which is unaffected.
...
The v1 handler decorators converted several return shapes into the wire result type; v2 `on_*` handlers return the fully constructed result (see [The low-level Server](advanced/low-level-server.md)), and a ported body that still returns a bare `dict` or list fails the request with an `MCPError` instead. If you want the conveniences back, use `MCPServer`, whose `@mcp.tool()` and `@mcp.resource()` still wrap return values.
...
The v1 `@server.call_tool()` decorator caught any exception the handler raised and returned it as `CallToolResult(isError=True)` carrying the exception text, so the model saw the error and could retry. A v2 `on_call_tool` handler has no such wrapper: the exception fails the `tools/call` request with a JSON-RPC error, the client's `call_tool()` raises `MCPError` (`Internal server error` on v2's default connection), and the model gets no tool result; see [The low-level Server](advanced/low-level-server.md).
...
async def call_tool(name: str, arguments: dict):
    raise ValueError("kaboom")  # client received CallToolResult(isError=True) with text "kaboom"
```
...
Raise `MCPError` only when the request itself should fail as a protocol error. `MCPServer`'s `@mcp.tool()` still converts other exceptions into `is_error=True` results, but only a `ToolError`'s message reaches the model; see [Exception messages from `@mcp.tool()` handlers no longer reach the model](#exception-messages-from-mcptool-handlers-no-longer-reach-the-model).

### Lowlevel `Server`: constructor parameters are now keyword-only

Every parameter after `name` is keyword-only, so passing `version` (or anything later) positionally raises `TypeError` at construction.
...
`Server[LifespanResultT, RequestT]` is now `Server[LifespanResultT]`. Drop the second type argument: a two-argument subscript fails type checking and raises `TypeError` where the annotation is evaluated at runtime.
...
The public `server.request_handlers` and `server.notification_handlers` dicts are gone. Register handlers with the constructor's `on_*` parameters or, after construction, with `add_request_handler` / `add_notification_handler`; look them up with `get_request_handler` / `get_notification_handler`, which return the registered entry (with `.handler` and `.params_type`) or `None`. Keys are method strings, not request types.
...
`handle_list_tools` uses the v2 `(ctx, params)` handler signature; see [The low-level Server](advanced/low-level-server.md#a-method-of-your-own) for `add_request_handler` and custom methods.
...
v1 answered a request the peer cancelled with a JSON-RPC error, `{"code": 0, "message": "Request cancelled"}`; v2 sends no response (no result, no error) in either direction: the server for a cancelled client request, the client for a cancelled server-initiated request such as sampling or elicitation. The one exception is the 2025-era streamable HTTP server transport, whose wire can end a request's stream only with a response: it terminates a cancelled request with `-32800` (`mcp.server.streamable_http.REQUEST_CANCELLED`) instead of the old `0`.

Callers that abandon a call through the SDK (cancelling the awaiting task or scope, or a per-request timeout) are unaffected. Code that hand-sends `notifications/cancelled` while another task still awaits the call breaks: the call no longer raises `McpError("Request cancelled")`; depending on the connection it hangs unanswered, fails with `MCPError` code `-32800` (2025-era streamable HTTP), or completes normally because the hand-sent cancel is not applied (e.g. the default in-process `Client(server)`). Cancel the scope or task awaiting the call instead; v2 signals the server for you and the handler is interrupted:
...
```python
with anyio.CancelScope() as scope:  # scope.cancel() abandons the call and interrupts the server-side handler
...
Passing `stateless=` to the lowlevel `Server.run()` raises `TypeError`; delete it. Stateless serving is a setting of the streamable-HTTP transport, which the lowlevel `Server` can now build itself:
...
app = server.streamable_http_app(stateless_http=True)  # new on the lowlevel Server
# or keep driving the session manager yourself (unchanged from v1):
manager = StreamableHTTPSessionManager(app=server, stateless=True)
```

Over any other stream pair, a pre-2026 client must now send `initialize` before its first request. On a `stateless_http=True` server a handler's server-initiated request (sampling, elicitation, roots) raises `NoBackChannelError` instead of hanging until the client's read timeout as it did in v1; see [Server-initiated sampling, elicitation, and roots raise `NoBackChannelError`](#server-initiated-sampling-elicitation-and-roots-raise-nobackchannelerror) and [Serving legacy clients](run/legacy-clients.md).
...
- The `sampling_callback`, `elicitation_callback`, and `list_roots_callback` client callbacks receive the non-generic `ClientRequestContext` from `mcp.client` (`session`, `request_id`, `meta`); the always-`None` `lifespan_context` and `request` are gone.
...
A leftover two-argument v1 annotation such as `Context[ServerSession, None]` still runs but silently types `lifespan_context` as `ServerSession`; three type arguments raise `TypeError`.
...
The `LifespanContextT`/`RequestT` TypeVars now live in `mcp.server.context`. `ServerRequestContext.request_id` is `RequestId | None` (`None` when the same context reaches a notification handler), so code passing `ctx.request_id` on as a definite `RequestId` needs a `None` check.
...
- **`ctx.session` is a new object on every request.** v1 kept one `ServerSession` per connection, so servers keyed per-client state on its identity (`WeakKeyDictionary[ServerSession, ...]`, `id(ctx.session)`, a set of captured sessions to notify later). Those idioms now silently misbehave: a session-keyed dict never finds an earlier entry, and a broadcast set gains a proxy per request and sends duplicates. Key on something connection-stable instead: on stateful streamable HTTP the `mcp-session-id` header (`ctx.headers` on `MCPServer`, `ctx.request.headers` in a lowlevel handler; 2026-07-28 clients are [sessionless](run/legacy-clients.md) and send none); on stdio the process serves a single client.
...
`ServerSession.elicit()` and `elicit_form()` renamed the `requestedSchema` parameter to `requested_schema`; keyword callers get `TypeError: ServerSession.elicit_form() got an unexpected keyword argument 'requestedSchema'`.
...
That `raise` no longer takes the session down: v2 logs an exception raised from `message_handler` and carries on, whereas on v1 it ended the receive loop and every later send raised `anyio.ClosedResourceError`.

Behavior changes on unchanged session code:

- **Callbacks and notifications run concurrently.** v1 handled one inbound message at a time, so callbacks ran inline and in order; v2 runs each server-initiated request callback and each notification delivery in its own task. They may interleave, a callback may itself send requests (that deadlocked in v1), an in-flight callback is interrupted when the server sends `notifications/cancelled`, and a `progress_callback` may run after its request has returned. Callbacks that need strict ordering must coordinate themselves.
...
- **After the connection closes, `send_request` raises `MCPError` (`CONNECTION_CLOSED`) and `send_notification` is dropped with a debug log** (the typed notification helpers included); v1 raised `anyio.ClosedResourceError` or `anyio.BrokenResourceError` from both. If that exception was your disconnect signal, probe with a request instead. `send_request` before entering the context manager now raises `RuntimeError` (v1 wrote the request and hung).
...
The sections below are client-side apart from [`stdio_server` keeps the protocol streams on private descriptors](#stdio_server-keeps-the-protocol-streams-on-private-descriptors); the streamable-HTTP lifespan change is under MCPServer ([lifespan entered once at manager startup](#streamable-http-lifespan-now-entered-once-at-manager-startup)).
...
`streamable_http_client` now yields `(read_stream, write_stream)` instead of a 3-tuple, so a three-name unpack fails with `ValueError: not enough values to unpack (expected 3, got 2)`. The `get_session_id` callback, the `StreamableHTTPTransport.get_session_id()` method behind it, and the `GetSessionIdCallback` alias are gone (`from mcp.client.streamable_http import GetSessionIdCallback` raises `ImportError`; inline `Callable[[], str | None]` if you still need the type).
...
`StreamableHTTPTransport` no longer exposes a `protocol_version` attribute; read the negotiated version from `session.protocol_version` on `ClientSession` or `client.protocol_version` on the high-level [`Client`](client/index.md). The header-name constant moved from `mcp.client.streamable_http` to `mcp.shared.inbound` under a new name (same value, `"mcp-protocol-version"`):
...
| Any of the above with a JSON-RPC error body | body ignored | body's error surfaced verbatim; an SDK server's unknown-session 404 arrives as `MCPError(-32600, 'Session not found')` |
...
A POSIX watchdog thread that polls fd 0 to detect a vanished client stops working. While `stdio_server()` (and so any stdio `MCPServer`) is serving, the protocol runs on private duplicates of the standard descriptors, with fd 0 pointing at the null device and fd 1 at stderr (both restored on exit). The null device never reports `POLLHUP` or `POLLERR`, so the wait no longer fires when the client closes the pipe (on macOS, where `poll()` does not support device files, it can instead return `POLLNVAL` at startup). Poll a duplicate of the descriptor taken before the server starts:
...
The provider sends a `client_credentials` grant with `private_key_jwt` client authentication rather than the `jwt-bearer` grant `RFC7523OAuthClientProvider` sent, and takes a pre-registered `client_id` plus `scope=` and `issuer=` (see [`scopes=` renamed to `scope=`](#scopes-renamed-to-scope-on-the-client-credentials-providers)) in place of `client_metadata` (no dynamic client registration). Relative to `JWTParameters`, `SignedJWTParameters` drops the `jwt_` prefix (`jwt_signing_key` → `signing_key`, `jwt_signing_algorithm` → `signing_algorithm`, `jwt_lifetime_seconds` → `lifetime_seconds`), renames `claims` to `additional_claims`, and has no `audience` (the provider supplies the authorization server's issuer); a prebuilt `JWTParameters(assertion=...)` becomes `assertion_provider=static_assertion_provider(token)`. See [OAuth clients](client/oauth-clients.md#machine-to-machine). For an enterprise ID-JAG `jwt-bearer` grant, use `IdentityAssertionOAuthProvider` ([Identity assertion](client/identity-assertion.md)). The `authorization_code` flow with `private_key_jwt` token-endpoint authentication has no v2 equivalent. The module also no longer re-exports `OAuthTokenError`; import it from `mcp.client.auth`.

### OAuth metadata URLs no longer gain a trailing slash

`OAuthMetadata`, `ProtectedResourceMetadata`, `OAuthClientMetadata`, `OAuthClientInformationFull`, and `AuthSettings` now set `url_preserve_empty_path=True`: a path-less URL parsed from a string (constructor argument or wire JSON) keeps its empty path instead of gaining a trailing slash.
...
`ClientCredentialsOAuthProvider` and `PrivateKeyJWTOAuthProvider` renamed the `scopes` keyword to `scope`; the value is still a single space-separated string, and an unchanged `scopes=` call raises `TypeError`.
...
provider = ClientCredentialsOAuthProvider(
    server_url, storage, client_id, client_secret, scope="read write", issuer="https://auth.example.com"
)
```

Also pass `issuer=`, the issuer URL of the authorization server that issued the credentials: token requests are then only built from discovered metadata naming that issuer, otherwise the flow stops with `OAuthFlowError`, and constructing either provider without it emits `MCPDeprecationWarning` (see [OAuth clients](client/oauth-clients.md#machine-to-machine)). With `token_endpoint_auth_method="client_secret_post"` the token request body now carries `client_id` next to `client_secret` ([RFC 6749](https://datatracker.ietf.org/doc/html/rfc6749) §2.3.1), so update any mock authorization server or test that pins the exact form body.
...
`OAuthClientProvider` (and the client-credentials providers built on it) now requires the authorization server metadata `issuer` to be string-equal to the URL discovery started from ([RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) section 3.3, [SEP-2468](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2468)): the entry taken from the protected resource metadata's `authorization_servers` list, or, when the MCP server publishes no protected resource metadata, the MCP server's own origin (`scheme://host[:port]`, with or without a trailing slash). v1 never compared the two, so a deployment whose values disagree (even by a trailing slash) authenticated under v1 and now fails discovery. For example, protected resource metadata advertising `"authorization_servers": ["https://as.example.com"]` against authorization server metadata with `"issuer": "https://as.example.com/"` raises:
...
The origin fallback itself now requires a clean miss: if a protected resource metadata URL answers `5xx` or `429` and none succeeds, discovery stops with `OAuthFlowError: Protected resource metadata request failed: HTTP <status>` where v1 fell through to the origin, so a server without that document must answer `404` for it.

### OAuth client requests `offline_access` and sends `prompt=consent`

When the authorization server's metadata lists `offline_access` in `scopes_supported` and the client's `grant_types` include `refresh_token` (the default), the client now appends `offline_access` to the requested scope ([SEP-2207](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2207)); whenever `offline_access` is in the scope, the authorization request also carries `prompt=consent`. Unchanged v1 code sends a different authorization URL:
...
End users see the provider's consent screen on every authorization instead of being re-authorized silently, an authorization server that ties refresh tokens to `offline_access` now issues them, and one that does not allow `offline_access` for the client can fail the flow with `invalid_scope`.
...
This also drops `refresh_token` from the Dynamic Client Registration payload, so authorization servers that gate refresh tokens on the registered grants stop issuing them. If `offline_access` is already in the selected scope (from a `WWW-Authenticate` challenge or a `scopes_supported` list), `prompt=consent` is sent regardless of `grant_types`; there is no separate switch for it.

### Dynamic Client Registration now sends `application_type`

The registration request now always includes `application_type` ([SEP-837](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/837)), defaulting to `"native"`, the correct value for clients with loopback (`localhost`) redirect URIs. A browser-based client served from a non-local host must set `"web"`, or an OIDC authorization server may reject its non-loopback redirect URIs:
...
On the server side, a custom `OAuthAuthorizationServerProvider.register_client` behind the SDK's registration endpoint now receives `client_secret_expires_at=0` (RFC 7591's "never expires") rather than `None` when a secret is issued and `ClientRegistrationOptions.client_secret_expiry_seconds` is unset; treat `0` as no expiry, not as a timestamp.
...
**Secret-method client records with no stored secret are rejected.** In v1, a client record whose `token_endpoint_auth_method` was `client_secret_post` or `client_secret_basic` but whose `client_secret` was `None` authenticated without a valid secret. v2 rejects it: `/token` returns 401 `invalid_client` and `/revoke` returns 401 `unauthorized_client`, both with "Client is registered for secret-based authentication but has no stored secret". Clients registered through `/register` are unaffected (secret-based registrations always receive a secret); hand-provisioned records need a stored secret or a public auth method.
...
Every request the SDK sends (client-to-server and server-to-client, at every negotiated protocol version) now includes `params._meta`, even where v1 sent no `params` at all; request ids also start at 1 instead of 0 and key order differs. No setting restores the v1 wire shape, so update fixtures, mock servers, and snapshot tests that assert on raw request bytes.

**Before (v1):** `ClientSession` against a 2025-11-25 server:
...
**After (v2):** the same `ClientSession` code (or `Client(..., mode="legacy")`) against the same server:
...
Unlike the helper, which always ran the pre-2026 `initialize` handshake, `Client(server)` negotiates 2026-07-28 by default, so unchanged sampling, elicitation, roots, logging, and change-notification code behaves differently: see [Behavior changes on v2's default connection](#behavior-changes-on-v2s-default-connection). Also on that connection: every result's `meta` carries an `io.modelcontextprotocol/serverInfo` stamp, so whole-result assertions such as `assert result == CallToolResult(...)` fail until you clear `result.meta` or compare fields; `send_ping()` raises `MCPError` `-32601` (Method not found), as do `set_logging_level()` and `subscribe_resource()`/`unsubscribe_resource()` against a lowlevel `Server` that handles them; and client-to-server progress notifications are not delivered, nor do handlers receive a `progress_token` (`report_progress()` delivers either way). `Client(server, mode="legacy")` restores the pre-2026 handshake and all of the above.

`create_client_server_memory_streams()` remains for driving a `ClientSession` by hand, but the streams it yields no longer expose anyio's `send_nowait`, `receive_nowait`, or `statistics()`.

### Deprecated v1 calls now warn (breaks warnings-as-errors test runs)

v2 still ships the APIs the 2026-07-28 spec retires, but using them now emits `MCPDeprecationWarning`, a `UserWarning` subclass exported from `mcp`: the `Context` logging helpers (`await ctx.info(...)` and friends), the sampling, roots, resource-subscription and client-to-server progress methods on the sessions, the lowlevel `Server`'s `on_set_logging_level=`-style parameters (at construction), and a few SDK helpers; [Deprecated features](deprecated.md) has the replacements. A test suite configured with `filterwarnings = ["error"]` therefore fails on unchanged v1 code. Inside an `@mcp.tool()` handler the raised warning surfaces as `CallToolResult(is_error=True)` with text `Error executing tool <name>`, and the `MCPDeprecationWarning` traceback appears only in the server log. Strict type checkers flag the same calls (`reportDeprecated`). Replace the calls, or keep the warnings visible but non-fatal with a `default` filter after the `error` entry:
...
v2's `Client` (including the in-process `Client(server)` that replaces v1's test helper) negotiates the 2026-07-28 protocol by default, so the v1 patterns below change behavior even though the server code is untouched. `Client(..., mode="legacy")` reproduces v1's `initialize` handshake and restores each of them (see [Protocol versions](protocol-versions.md)); connections negotiated at 2025-11-25 or earlier (a `ClientSession` you `initialize()` yourself, any 2025-era peer) are unaffected.

### Server-initiated sampling, elicitation, and roots raise `NoBackChannelError`

A handler that reaches back to the client mid-request (`ctx.elicit()`, `ctx.elicit_url()`, `ctx.session.create_message()`, `ctx.session.list_roots()`, or any other `ServerSession` request helper) raises `NoBackChannelError` where the connection has no server-to-client channel: every 2026-07-28 connection (the protocol has no server-initiated requests), a legacy session against a `stateless_http=True` server, and `ctx.elicit()` / `ctx.elicit_url()` on a legacy session against a `json_response=True` server (their request-scoped channel is a single JSON body). In the last two cases the v1 call hung until a client-side timeout fired; v2 fails fast. So an unchanged v1 sampling or elicitation tool fails on its first `Client(server)` test, and `sampling_callback=` / `elicitation_callback=` / `list_roots_callback=` change nothing: no request ever reaches the client.

`NoBackChannelError` (`mcp.shared.exceptions`) subclasses `MCPError` with code `-32600` and message `Cannot send '<method>': this transport context has no back-channel for server-initiated requests.`. Raised inside a handler it reaches the client as an `MCPError`, not `CallToolResult(is_error=True)` (see [`MCPError` raised from an `@mcp.tool()` handler now surfaces as a JSON-RPC error](#mcperror-raised-from-an-mcptool-handler-now-surfaces-as-a-json-rpc-error)). Notification helpers never raise it: undeliverable notifications are dropped instead (see the next two sections). `UrlElicitationRequiredError` is unaffected; it is an error response, not a request.
...
- **Port to the era-portable form**: a `Resolve(...)`-backed parameter whose resolver returns `Elicit`, `Sample`, or `ListRoots` makes the server return the question instead of pushing it. The SDK elicits directly on a legacy connection and drives the `InputRequiredResult` round trip at 2026-07-28, with one handler body for both eras; see [Elicitation](handlers/elicitation.md), [Sampling and roots](handlers/sampling-and-roots.md), and [Multi-round-trip requests](handlers/multi-round-trip.md).
...
@mcp.tool()
async def book_table(date: str, answer: Annotated[ElicitationResult[Confirmation], Resolve(ask_to_confirm)]) -> str:
...
A migrated test whose `logging_callback` received every `notifications/message` in v1 now receives none: on a 2026-07-28 connection, protocol logging is a per-request opt-in, so `ctx.info(...)` / `ctx.log(...)` on `MCPServer`'s `Context` and `ctx.session.send_log_message(...)` are dropped (debug-logged) unless the request's `_meta` carries `io.modelcontextprotocol/logLevel`, and then only messages at that level or above are sent. `logging/setLevel` is gone there too: `client.set_logging_level(...)` raises `MCPError` (`-32601`, Method not found) even when the server registered a handler. Opt in with `log_level=` on `Client` (a per-call `meta=` entry with the same key overrides it), or pin `Client(server, mode="legacy", ...)` to keep v1's push delivery. See [Client callbacks](client/callbacks.md#the-notification-callbacks).
...
On a 2026-07-28 connection the v1 helpers `ctx.session.send_tool_list_changed()`, `send_prompt_list_changed()`, `send_resource_list_changed()`, and `send_resource_updated(uri)` are dropped with a debug log, so a ported `Client(server)` test's `message_handler` never sees them: there the four change notifications reach a client only through a `subscriptions/listen` stream it opened ([`client.listen(...)`](client/subscriptions.md)). Their 2026-07-28 counterparts are `await ctx.notify_tools_changed()` / `notify_prompts_changed()` / `notify_resources_changed()` / `notify_resource_updated(uri)` on `MCPServer`'s `Context` ([Subscriptions](handlers/subscriptions.md)); neither the helper nor its counterpart reaches the other era's clients, so to notify everyone call both ([Serving legacy clients](run/legacy-clients.md#where-your-code-actually-forks)).
...
On a lowlevel `Server`, publish to your own `SubscriptionBus` (`await bus.publish(ToolsListChanged())` from `mcp.server.subscriptions`) served by `ListenHandler(bus)` on `on_subscriptions_listen=`; see [the low-level composition](handlers/subscriptions.md#the-low-level-composition).

## Need Help?

If you encounter issues during migration:

1. Check the [API Reference](api/mcp/index.md) for current signatures
2. Review the [examples](https://github.com/modelcontextprotocol/python-sdk/tree/main/examples)
3. Open an issue on [GitHub](https://github.com/modelcontextprotocol/python-sdk/issues) if something here is wrong or missing
