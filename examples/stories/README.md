...
| [`mrtr`](mrtr/) | `InputRequiredResult` round-trip: the `Client` auto-loop, a manual session-level loop, and `RequestStateSecurity` sealing `requestState` (tamper → one frozen error) | current |
