...
def test_generates_uv_run_command(config_dir: Path) -> None:
...
def test_file_spec_without_object_suffix(config_dir: Path) -> None:
...
def test_with_packages_sorted_and_deduplicated(config_dir: Path) -> None:
...
def test_with_editable_adds_flag(config_dir: Path, tmp_path: Path) -> None:
...
def test_env_vars_written(config_dir: Path) -> None:
...
def test_existing_env_vars_merged_new_wins(config_dir: Path) -> None:
...
def test_existing_env_vars_preserved_without_new(config_dir: Path) -> None:
...
def test_other_servers_preserved(config_dir: Path) -> None:
...
def test_raises_when_config_dir_missing(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_uv_path(monkeypatch: pytest.MonkeyPatch, which_result: str | None, expected: str) -> None:
