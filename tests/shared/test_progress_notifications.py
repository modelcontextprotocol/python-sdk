...
async def test_bidirectional_progress_notifications():
    ...
    # Store server session reference
    serv_sesh: ServerSession | None = None

    # Define handlers
    async def on_progress_notification(notification: types.ProgressNotification) -> None:
...
async def test_progress_context_manager():
    ...
    server = Server(name="ProgressContextTestServer", on_progress_notification=on_progress_notification)
...
async def test_progress_callback_exception_logging():
    ...
    server = Server(name="TestProgressServer", on_list_tools=on_list_tools, on_call_tool=on_call_tool)
