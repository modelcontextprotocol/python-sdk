...
# session manager's per-session task-group cancel can race the per-request cleanup). The old
# socket-based version of this file ran the transport in a separate process and so never
# observed these `__del__`-time ResourceWarnings; running in-process via the streaming bridge
# does. The fixes live in `src/` on `main` and are out of scope for this tests-only change.
# The filters are scoped to anyio's `MemoryObject*Stream` leak signature so an unrelated leak
# still fails the suite; tests/interaction/conftest.py applies the same pair for the same reason.
pytestmark = [
...
@asynccontextmanager
async def _server_lifespan(_server: Server[ServerState, Request]) -> AsyncIterator[ServerState]:
    yield ServerState()


def _create_server() -> Server[ServerState, Request]:
    server: Server[ServerState, Request] = Server(SERVER_NAME, lifespan=_server_lifespan)
...
async def running_app(
    ...
    server: Server[Any, Request] | None = None,
...
async def test_accept_header_incompatible(basic_app: Starlette, accept_header: str) -> None:
    """Accept headers that do not literally include both required media types are rejected for SSE mode.

    (v1 matches Accept media types literally; wildcard support is a main-only change, #2152.)
    """
    async with make_client(basic_app) as client:
        response = await client.post(
            "/mcp",
...
async def initialized_client_session(basic_app: Starlette) -> AsyncIterator[ClientSession]:
    ...
    async with (
        ...
        streamable_http_client(f"{BASE_URL}/mcp", http_client=http_client) as (read_stream, write_stream, _),
        ...
        async with streamable_http_client(f"{BASE_URL}/mcp", terminate_on_close=False, http_client=httpx_client) as (
            ...
            async with ClientSession(  # pragma: no branch
                ...
                headers: dict[str, Any] = {
                    ...
                    MCP_PROTOCOL_VERSION_HEADER: result.protocolVersion,
                }
...
def _create_context_server() -> Server[dict[str, Any], Request]:
    server: Server[dict[str, Any], Request] = Server("ContextAwareServer")
    ...
    async def handle_call_tool(name: str, args: dict[str, Any]) -> list[TextContent]:
        ...
        if name == "echo_headers":
            return [TextContent(type="text", text=json.dumps(dict(ctx.request.headers)))]
...
async def test_streamable_http_multiple_reconnections() -> None:
    ...
    server: Server[dict[str, Any], Request] = Server("multi_reconnect_server")

    @server.call_tool()
    async def handle_call_tool(name: str, args: dict[str, Any]) -> list[TextContent]:
        ctx = server.request_context
        assert name == "multi_close_tool"
        ...
        return [TextContent(type="text", text="Completed 3 checkpoints")]
...
async def test_streamable_http_client_does_not_mutate_provided_client(basic_app: Starlette) -> None:
    ...
    async with make_client(basic_app, headers=original_headers) as custom_client:
        ...
        # These checks deliberately sit after the streamable_http_client context
        # exits (a teardown-time mutation would otherwise escape them), which on
        # Python 3.11 places them in the post-teardown trace-loss shadow
        # (python/cpython#106749): they run and assert on every leg but go
        # unmeasured on 3.11 cells, hence the lax exclusions.
        # If accept header exists, it should still be httpx default, not MCP's
        if "accept" in custom_client.headers:  # pragma: lax no cover
...
async def test_streamable_http_client_mcp_headers_override_defaults(context_app: Starlette) -> None:
    ...
    async with make_client(context_app) as client:
        ...
        async with streamable_http_client(f"{BASE_URL}/mcp", http_client=client) as (read_stream, write_stream, _):
            async with ClientSession(read_stream, write_stream) as session:  # pragma: no branch
                await session.initialize()
...
def test_streamable_http_transport_deprecated_params_ignored() -> None:
    """Deprecated parameters passed to StreamableHTTPTransport are accepted but ignored."""
...
@pytest.mark.anyio
async def test_streamablehttp_client_deprecation_warning(basic_app: Starlette) -> None:
    """The old streamablehttp_client() function issues a deprecation warning."""
    ...
    def in_process_client_factory(
        ...
        return httpx.AsyncClient(
            transport=StreamingASGITransport(basic_app),
            base_url=BASE_URL,
            headers=headers,
            timeout=timeout,
            auth=auth,
    ...
    with pytest.warns(DeprecationWarning, match="Use `streamable_http_client` instead"):
        async with streamablehttp_client(  # pyright: ignore[reportDeprecated]
            f"{BASE_URL}/mcp", httpx_client_factory=in_process_client_factory
        ) as (
            read_stream,
            write_stream,
            _,
