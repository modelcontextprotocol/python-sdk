"""Tests for the httpx helpers the client transports are built on."""
...
def test_default_client_uses_mcp_timeouts_and_httpx_redirect_default():
    ...
    assert client.timeout == httpx.Timeout(30.0, read=300.0)
...
def _recording_client(
...
) -> tuple[httpx.AsyncClient, list[str], list[bool]]:
    ...
    def serve(request: httpx.Request) -> httpx.Response:
        ...
        if str(request.url) in redirects:
            ...
            return httpx.Response(status, headers={"location": location}, stream=_Body(b"moved", closed))
        return httpx.Response(200, text=request.content.decode() or "ok")

    return httpx.AsyncClient(transport=httpx.MockTransport(serve), **client_kwargs), received, closed
...
async def test_redirect_within_origin_is_followed_with_method_and_body(url: str, location: str):
    ...
    307 keeps the method and body (SDK-defined policy; the re-send itself is httpx's)."""
    ...
    assert received == [f"POST {url}", f"POST {httpx.URL(url).join(location)}"]
    assert closed == [True]
...
async def test_redirect_outside_origin_is_not_followed(location: str):
    """A redirect to another origin is handed back unfollowed, the way httpx hands back a redirect
...
async def test_method_changing_redirect_of_a_post_is_not_followed(status: int):
    """httpx turns a POST into a body-less GET for 301/302/303, which would drop the message, so a
...
async def test_same_origin_redirect_of_a_get_is_followed_for_every_redirect_status(status: int):
    ...
    within the origin (SDK-defined policy over httpx's method rules)."""
...
async def test_redirect_location_with_userinfo_is_not_followed():
    ...
    httpx would otherwise send that userinfo as Basic auth (SDK-defined)."""
    url = "http://mcp.example/mcp"
...
async def test_sse_within_origin_sends_event_stream_headers_and_caller_headers():
    """The SSE form asks for an event stream exactly as httpx_sse.aconnect_sse() does, merged case-insensitively
    with the caller's headers, and yields an EventSource over the final response."""
    seen: list[httpx.Headers] = []

    def serve(request: httpx.Request) -> httpx.Response:
        seen.append(request.headers)
        return httpx.Response(200, headers={"content-type": "text/event-stream"}, text="data: hello\n\n")
    ...
    async with client:
        async with sse_within_origin(client, "http://mcp.example/sse") as source:
            events = [event.data async for event in source.aiter_sse()]
...
async def test_auth_flow_requests_are_not_redirected():
    """Requests an httpx Auth flow issues while a transport request is in flight (a token refresh,
    say) inherit the per-request no-follow setting, so a redirect on them is handed back to the
    auth flow rather than followed (httpx behaviour the transports rely on)."""
    received: list[str] = []

    class TokenThenRequest(httpx.Auth):
        async def async_auth_flow(self, request: httpx.Request) -> AsyncGenerator[httpx.Request, httpx.Response]:
            token_response = yield httpx.Request("POST", "http://mcp.example/token", content=b"grant")
    ...
    def serve(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/token":
            return httpx.Response(307, headers={"location": "http://other.example/token"})
        return httpx.Response(200, text=request.headers["x-token-status"])

    client = httpx.AsyncClient(transport=httpx.MockTransport(serve), auth=TokenThenRequest(), follow_redirects=True)
    async with client:
