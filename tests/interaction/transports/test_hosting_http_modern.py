...
async def test_modern_tools_call_returns_result_type_complete_without_initialize() -> None:
    ...
    pins the lazy-upgrade JSON arm: a handler that emits nothing before its result never commits
    SSE.
    """
    body = {
        "jsonrpc": "2.0",
...
async def test_modern_server_discover_returns_capabilities_and_supported_versions() -> None:
    ...
    travels as the io.modelcontextprotocol/serverInfo result `_meta` stamp. Also pins the SEP-2549
    caching hints the entry stamps on the discover result (the SDK defaults `ttlMs 0` /
    `cacheScope private`) under `caching:hints:server-discover`. Asserted at the wire because
