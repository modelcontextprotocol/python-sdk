...
async def connect_with_oauth(
    ...
    *,
    ...
    `httpx2.AsyncClient` issues, including those yielded from inside the auth flow. `mode`
    defaults to "legacy" because most auth tests snapshot the initialize-handshake HTTP shape;
    pass a modern version or "auto" to run the flow on the 2026-07-28 entry.
    ...
    async with AsyncExitStack() as stack:
        ...
        client = await stack.enter_async_context(
            Client(streamable_http_client(f"{BASE_URL}/mcp", http_client=http_client), mode=mode)
        )
