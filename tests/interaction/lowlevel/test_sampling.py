...
async def test_create_message_round_trip(connect: Connect) -> None:
    ...
    async def sampling_callback(
        ...
        return CreateMessageResult(
            role="assistant",
            content=TextContent(type="text", text="Hello to you too."),
            model="mock-llm-1",
            stopReason="endTurn",
        )
    ...
    assert result == snapshot(
        CallToolResult(content=[TextContent(type="text", text="mock-llm-1/endTurn: Hello to you too.")])
    )
...
async def test_create_message_request_with_image_content_reaches_callback(connect: Connect) -> None:
    ...
    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="described image/png (aW1n)")]))
    assert received == snapshot(
        [
            CreateMessageRequestParams(
                _meta={},
...
async def test_create_message_result_with_image_content_returns_to_handler(connect: Connect) -> None:
    ...
    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="mock-vision-1: image/png Y2F0")]))
...
async def test_create_message_callback_error(connect: Connect) -> None:
    ...
    assert result == snapshot(
        CallToolResult(content=[TextContent(type="text", text="-1: User rejected sampling request")])
    )
...
async def test_create_message_without_callback_is_error(connect: Connect) -> None:
    ...
    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="-32600: Sampling not supported")]))
...
async def test_create_message_with_tools_is_rejected_for_unsupporting_client(connect: Connect) -> None:
    ...
    assert result == snapshot(
        CallToolResult(
            content=[TextContent(type="text", text="-32602: Client does not support sampling tools capability")]
        )
    )
            ...
            await ctx.session.create_message(
                messages=[
                    SamplingMessage(
                        ...
                        content=[
                            ToolResultContent(
                                type="tool_result", toolUseId="call-1", content=[TextContent(type="text", text="42")]
                            ),
                            TextContent(type="text", text="Also, a comment alongside the result."),
                        ],
                    )
                ],
                max_tokens=100,
    ...
    assert result == snapshot(
        CallToolResult(
            content=[
                TextContent(
                    type="text",
                    text="ValueError: The last message must contain only tool_result content if any is present",
                )
            ]
        )
    )
...
async def test_create_message_request_with_audio_content_reaches_callback(connect: Connect) -> None:
    ...
    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="transcribed audio/wav (c25k)")]))
    assert received == snapshot(
        [
            CreateMessageRequestParams(
                _meta={},
...
async def test_create_message_result_with_audio_content_returns_to_handler(connect: Connect) -> None:
    ...
    assert result == snapshot(
        CallToolResult(content=[TextContent(type="text", text="mock-audio-1: audio/wav aGVsbG8=")])
    )
...
async def test_create_message_with_list_valued_message_content_reaches_callback(connect: Connect) -> None:
        ...
        result = await ctx.session.create_message(
            messages=[
                SamplingMessage(
                    role="user",
                    content=[
                        TextContent(type="text", text="Caption this image."),
    ...
    async def sampling_callback(
        ...
        return CreateMessageResult(
            role="assistant", content=TextContent(type="text", text=f"{len(content)} blocks"), model="mock-llm-1"
        )

    async with connect(server, sampling_callback=sampling_callback) as client:
        result = await client.call_tool("caption", {})

    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="2 blocks")]))
    assert received == snapshot(
        [
            CreateMessageRequestParams(
                _meta={},
        ...
        try:
            await ctx.session.create_message(
                messages=[
                    SamplingMessage(
                        role="assistant",
                        content=[ToolUseContent(type="tool_use", id="call-1", name="weather", input={})],
                    ),
                    SamplingMessage(
                        role="user",
                        content=[
...
async def test_array_content_result_for_a_tool_free_request_surfaces_as_a_validation_error(connect: Connect) -> None:
    ...
    assert result == snapshot(CallToolResult(content=[TextContent(type="text", text="ValidationError")]))
