...
# Source: https://github.com/modelcontextprotocol/modelcontextprotocol/blob/6d441518de8a9d5adbab0b10a76a667a63f90665/schema/draft/schema.json
# Protocol version: 2026-07-28   Generator: datamodel-code-generator 0.57.0
# Regenerate: uv run --frozen python scripts/update_spec_types.py draft [--sha <new-sha>]
...
JSONValue = TypeAliasType("JSONValue", "JSONObject | list[JSONValue] | (str | int | bool)")
...
class ElicitRequestURLParams(OracleModel):
...
class ElicitResult(OracleModel):
    ...
    """
    content: dict[str, list[str] | str | int | bool] | None = None
    """
...
class ElicitationCompleteNotification(OracleModel):
...
class InternalError(OracleModel):
...
class InvalidParamsError(OracleModel):
...
class InvalidRequestError(OracleModel):
...
class LegacyTitledEnumSchema(OracleModel):
...
class MethodNotFoundError(OracleModel):
    ...
    A request that requires a client capability the client did not declare is signalled instead by MissingRequiredClientCapabilityError (`-32003`).
    """
...
class NotificationParams(OracleModel):
...
class ResourceUpdatedNotificationParams(OracleModel):
...
class SubscriptionFilter(OracleModel):
...
class SubscriptionsAcknowledgedNotificationParams(OracleModel):
...
class TitledMultiSelectEnumSchema(OracleModel):
...
class TitledSingleSelectEnumSchema(OracleModel):
...
class ToolUseContent(OracleModel):
...
class UnsupportedProtocolVersionError(OracleModel):
...
class UntitledMultiSelectEnumSchema(OracleModel):
...
class UntitledSingleSelectEnumSchema(OracleModel):
...
class CacheableResult(OracleModel):
...
class CancelledNotificationParams(OracleModel):
...
class CompleteResultResponse(OracleModel):
...
class JSONRPCErrorResponse(OracleModel):
...
class JSONRPCResultResponse(OracleModel):
...
class ProgressNotificationParams(OracleModel):
...
class ResourceLink(OracleModel):
    ...
    Note: resource links returned by tools are not guaranteed to appear in the results of ListResourcesRequestresources/list requests.
    """
...
class ResourceUpdatedNotification(OracleModel):
    """A notification from the server to the client, informing it that a resource has changed and may need to be read again. This is only sent for resources the client opted in to via the `resourceSubscriptions` field of a SubscriptionsListenRequestsubscriptions/listen request."""
...
class SubscriptionsAcknowledgedNotification(OracleModel):
...
class ElicitRequestFormParams(OracleModel):
...
class ListPromptsResultResponse(OracleModel):
...
class ListResourceTemplatesResultResponse(OracleModel):
...
class ListResourcesResultResponse(OracleModel):
...
class ListToolsResultResponse(OracleModel):
...
class ToolResultContent(OracleModel):
...
class CallToolRequestParams(OracleModel):
...
class CallToolResultResponse(OracleModel):
...
class CompleteRequestParams(OracleModel):
...
class CreateMessageRequestParams(OracleModel):
...
class DiscoverRequest(OracleModel):
...
class DiscoverResult(OracleModel):
...
class DiscoverResultResponse(OracleModel):
...
class GetPromptRequestParams(OracleModel):
...
class GetPromptResultResponse(OracleModel):
...
class InputRequiredResult(OracleModel):
...
class InputResponseRequestParams(OracleModel):
    model_config = ConfigDict(
        extra="allow",
    )
    meta: Annotated[RequestMetaObject, Field(alias="_meta")]
...
class MissingRequiredClientCapabilityError(OracleModel):
...
class PaginatedRequestParams(OracleModel):
...
class ReadResourceRequestParams(OracleModel):
...
class ReadResourceResultResponse(OracleModel):
...
class RequestMetaObject(OracleModel):
    ...
    """
    If specified, the caller is requesting out-of-band progress notifications for this request (as represented by ProgressNotificationnotifications/progress). The value of this parameter is an opaque token that will be attached to any subsequent notifications. The receiver is not obligated to provide these notifications.
    """


class RequestParams(OracleModel):
...
class ResourceRequestParams(OracleModel):
...
class SubscriptionsListenRequest(OracleModel):
...
class SubscriptionsListenRequestParams(OracleModel):
