...
from mcp.server.runner import ServerRunner, _extract_meta, aclose_shielded, otel_middleware
...
async def connected_runner(
    ...
    *,
    ...
    if stateless or not has_standalone_channel:
        connection = Connection.from_envelope(LATEST_PROTOCOL_VERSION, None, None)
    else:
        connection = Connection.for_loop(server_d, session_id=session_id)
    runner = ServerRunner(
        server=server,
