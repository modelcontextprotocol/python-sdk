...
# arm and an array-content arm with the same top-level keys (SDK-defined split);
# selection tries every structurally matching arm, best match first, so a body
# its best-looking arm rejects still parses when a sibling accepts it.


@pytest.mark.parametrize("version", [V1, "2025-03-26", "2025-06-18", V4, D])
...
def test_single_content_sampling_body_parses_as_the_single_content_arm() -> None:
    ...
    body = {"role": "assistant", "content": {"type": "text", "text": "hi"}, "model": "example-model"}
...
def test_result_body_rejected_with_the_best_matching_arms_errors_when_no_arm_validates() -> None:
    """A body keyed like a discover result but missing required
