...
async def test_enable_tasks_skips_default_handlers_when_custom_registered() -> None:
    ...
    # Register custom handlers via enable_tasks with on_* kwargs
    async def custom_get_task(ctx: ServerRequestContext[Any], params: Any) -> GetTaskResult:
        raise NotImplementedError

    async def custom_get_task_result(ctx: ServerRequestContext[Any], params: Any) -> GetTaskPayloadResult:
        raise NotImplementedError

    async def custom_list_tasks(ctx: ServerRequestContext[Any], params: Any) -> ListTasksResult:
        raise NotImplementedError

    async def custom_cancel_task(ctx: ServerRequestContext[Any], params: Any) -> CancelTaskResult:
        raise NotImplementedError

    # Enable tasks with custom handlers
    server.experimental.enable_tasks(
        on_get_task=custom_get_task,
        on_task_result=custom_get_task_result,
        on_list_tasks=custom_list_tasks,
        on_cancel_task=custom_cancel_task,
    )

    # Verify our custom handlers are still registered (not replaced by defaults)
    assert "tasks/get" in server._request_handlers
    assert "tasks/result" in server._request_handlers
    assert "tasks/list" in server._request_handlers
    assert "tasks/cancel" in server._request_handlers
